...
$(document).ready(function () {
  setTimeout(() => {
    ...
    function dropSubmenu(event) {
      ...
      let subPage = li.querySelector("#sub-page");
      let ul = li.querySelector("ul._navbar__submenu");


      if (subPage) {
        subPage.classList.toggle("_navbar__btn-page--active");
      }

      if (ul) {
        ul.classList.toggle("_navbar__submenu--active");
        console.log("Submenu toggled:", ul);
      }

      if (elementEvent.classList.contains("header__container")) {
        dropMenu();
      }
    }
    ...
    if (!desktop) {
      ...
      divLogo.innerHTML = `<img id="dnn_dnnLOGO_imgLogo" src="${imgLogo.src || ""
        }" alt="${imgLogo.alt || ""}"/>`;
